...
ENV INSTALL_PATH /guessing
# The name of the application is guessing and while there
...
# having to CD into the /guessing folder every time would be
# lame, so instead we can set the WORKDIR to be /guessing.
#
...
# guessing/ path inside of the Docker image.
#
# It copies it to /guessing because of the WORKDIR being set.
#
...
# /guessing folder inside of the Docker image.
#
